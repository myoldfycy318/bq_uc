...
public class Authentic {
	...
	private void flush() {
		synchronized(Authentic.class){
			try{
				if(token.isExpire()){
					if(Global.getAppClientId()!=null&&Global.getAppClientSecret()!=null){
						Map<String,Object>param = new HashMap<String,Object>();
						param.put("grant_type","client_credentials");
						param.put("client_id",Global.getAppClientId());
						param.put("client_secret",Global.getAppClientSecret());
						TalkNode res = service.request(Global.getUrlToken(),Global.HTTP_METHOD_POST,param,null,null);
