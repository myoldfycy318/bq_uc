...
public class BiReportLogEntity {
    ...
    //统计类型 gameChange --> 游戏转化表(手游部分) webGame --> 游戏转化表(H5 页游部分)  platform --> 平台统计表
    ...
    public boolean validate(boolean flag) {
        ...
        if (flag && StringUtils.isBlank(sysType)) {//true = 游戏转化表
            return false;
        }
        ...
        if (!flag && StringUtils.isBlank(platformType)) { //false = 平台报表
            return false;
        }
        return true;
    }
